...
def _convert_message_to_text(message: BaseMessage) -> str:
    """LangChain 메시지를 토큰과 함께 연결된 텍스트 문자열로 변환합니다.

    - 사용자 메시지(HumanMessage)는 "User: " 토큰을,
    - 어시스턴트 메시지(AIMessage)는 "Assistant: " 토큰을 접두사로 붙입니다.
    - 그 외의 메시지 타입은 역할 이름에 따라 적절한 토큰을 사용합니다.
    """
    # HumanMessage가 ChatMessage의 서브클래스라면 보다 구체적인 타입부터 검사합니다.
    if isinstance(message, HumanMessage):
        return f"User: {message.content}"
    elif isinstance(message, AIMessage):
        content = message.content

        # 추가 정보: function_call이 있으면 내용을 보완합니다.
        if "function_call" in message.additional_kwargs:
            func_call = message.additional_kwargs["function_call"]
            # 내용이 비어있다면 함수 호출 정보를 대체하도록 합니다.
            if content == "":
                content = f"[Function Call: {func_call}]"
            else:
                content += f" [Function Call: {func_call}]"

        # 추가 정보: tool_calls가 있으면 텍스트에 포함합니다.
        if message.tool_calls or message.invalid_tool_calls:
            tool_calls = ""
            if message.tool_calls:
                tool_calls += ", ".join(
                    str(_lc_tool_call_to_fireworks_tool_call(tc))
            ...
            if message.invalid_tool_calls:
                if tool_calls:
                    tool_calls += ", "
                tool_calls += ", ".join(
                    str(_lc_invalid_tool_call_to_fireworks_tool_call(tc))
                    for tc in message.invalid_tool_calls
                )
            content += f" [Tool Calls: {tool_calls}]"
        elif "tool_calls" in message.additional_kwargs:
            content += f" [Tool Calls: {message.additional_kwargs['tool_calls']}]"

        return f"Assistant: {content}"
    # ChatMessage가 HumanMessage나 AIMessage의 구체적 타입이 아니라면 role을 확인합니다.
        ...
        if role_lower == "user":
            ...
            token = f"{message.role.capitalize()}: "
        return f"{token}{message.content}"
    elif isinstance(message, SystemMessage):
        return f"System: {message.content}"
    elif isinstance(message, FunctionMessage):
        return f"Function: {message.name}: {message.content}"
    elif isinstance(message, ToolMessage):
        return f"Tool: {message.content} (ID: {message.tool_call_id})"
    else:
        raise TypeError(f"Got unknown type {message}")
...
def _convert_chunk_to_message_chunk(
    ...
    if default_class == AIMessageChunk:
...
class DeepSeekR1_Continue(BaseChatModel):
    ...
    def validate_environment(self) -> Self:
        ...
        if not self.client:
            self.client = Fireworks(**client_params).completions
        if not self.async_client:
            self.async_client = AsyncFireworks(**client_params).completions
        if self.max_retries:
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        message_text, params = self._create_message_text(messages, stop)
        params = {**params, **kwargs, "stream": True}

        default_chunk_class: type[BaseMessageChunk] = AIMessageChunk
        for chunk in self.client.create(prompt=message_text, **params):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        response = self.client.create(prompt=message_text, **params)
        return self._create_chat_result(response)

    def _create_message_text(
        self, messages: List[BaseMessage], stop: Optional[List[str]]
    ) -> Tuple[str, Dict[str, Any]]:
        params = self._default_params
        if stop is not None:
            params["stop"] = stop
        message_text = "\n\n".join([_convert_message_to_text(m) for m in messages])
        return message_text, params
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        default_chunk_class: type[BaseMessageChunk] = AIMessageChunk
        async for chunk in self.async_client.acreate(prompt=message_text, **params):
            if not isinstance(chunk, dict):
                chunk = chunk.model_dump()
            if len(chunk["choices"]) == 0:
                continue
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        response = await self.async_client.acreate(prompt=message_text, **params)
        return self._create_chat_result(response)
