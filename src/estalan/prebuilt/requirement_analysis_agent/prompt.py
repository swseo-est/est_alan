...
- **중복 방지**: state에 동일한 속성(카테고리+상세내용)이 이미 존재한다면 새로운 요구사항을 만들지 말고 update 도구로 수정하세요.
...
**중복 체크 규칙**:
1. 새로운 요구사항을 생성하기 전에 반드시 현재 state의 기존 요구사항들을 확인하세요.
2. 동일한 카테고리와 유사한 상세내용을 가진 요구사항이 이미 있다면 create_requirement 대신 update_requirement를 사용하세요.
3. 완전히 동일한 요구사항은 중복 생성하지 마세요.
4. 요구사항 생성 시 기존 요구사항과의 차이점을 명확히 구분하여 기록하세요.
...
1. **상태 관리**
   ...
   - 새로운 요구사항 생성 전에 기존 요구사항과의 중복을 반드시 확인하세요.
